...
function webstop()
{
    ...
    // Convert naptan_code requests into atco_code requests
    $sql = "SELECT atco_code from naptan_stop_point where naptan_code = '".trim($in_locations)."'";
    $row1 = executePDOQueryScalar($sql, $conn);
    if (isset($row1["atco_code"]))
        $in_locations = $row1["atco_code"];
...
function webstop_countdowns($in_location, $in_mode)
{
    ...
    SELECT location.location_id, 
                ...
                service_patt.dest_id, 
                    ...
                    timetable_journey_live.route_id, 
                    timetable_journey_live.trip_status, 
                    timetable_journey_live.start_code, 
                    timetable_journey_live.fact_id schedule_id, 
                    timetable_journey.etm_trip_no trip_no, 
                    timetable_journey_live.vehicle_id, 
                    timetable_visit_live.sequence, 
                    timetable_journey.ext_timetable_id pub_ttb_id, 
                    ...
                    unit_status.gps_time,
                    unit_status.gpslat,
                    unit_status.gpslong
            FROM timetable_journey_live, timetable_visit_live, timetable_journey, publish_tt, service, service_patt, route, operator, vehicle,
                    t_dcd_param, location, outer unit_status
            WHERE 1 = 1
            AND timetable_visit_live.journey_fact_id = timetable_journey_live.fact_id
            AND location.location_id = timetable_visit_live.location_id
            AND timetable_journey_live.vehicle_id = vehicle.vehicle_id
            AND timetable_journey_live.route_id = route.route_id
            ...
            AND timetable_visit_live.location_id = t_dcd_param.location_id
            AND service.service_id = service_patt.service_id
            AND timetable_visit_live.sequence = service_patt.rpat_orderby
            AND timetable_journey_live.timetable_id = timetable_journey.timetable_id
            AND timetable_journey.ext_timetable_id = publish_tt.pub_ttb_id
            AND publish_tt.service_id = service.service_id
            AND unit_status.build_id = vehicle.build_id
    ...
    // Consider each timetable_journey_live relevant to the current display_point
    ...
    while(is_array($row))
    {
            ...
            $wr_act_rte_loc["sequence"] = $row["sequence"]; 
            ...
            $wr_dcd_countdown["sequence"] = $wr_act_rte_loc["sequence"];
            ...
            $wr_unit_status["gps_time"] = $row["gps_time"];
            $wr_unit_status["gpslat"] = $row["gpslat"];
            $wr_unit_status["gpslong"] = $row["gpslong"];
            ...
            $txt .=  " S:". trim($wr_vehicle["vehicle_code"]). 
                    ...
                    " O:". trim($wr_dcd_countdown["sequence"]).
            ...
            // //-------------------------------------------------
            $sql = "SELECT MAX(sequence) last_stop
                FROM timetable_visit_live
                WHERE timetable_visit_live.journey_fact_id = ".$wr_act_rte["schedule_id"];
            $row1 = executePDOQueryScalar( $sql, $conn );
           
            $l_last_stop = $row1["last_stop"]; 
            if ( !$row1 || ( $l_last_stop == $wr_dcd_countdown["sequence"] && $wr_act_rte["start_code"] != "CONT" ) )
            {
            ...
            // ------------------------------------------------------------
            if ( $wr_dcd_countdown["sequence"] == 1 && ( !$lr_dcd_param["countdown_dep_arr"] || $lr_dcd_param["countdown_dep_arr"] != "D" ) ) {
                $txt = trim($txt).  "!LOC1->D";
            ...
            if ( $wr_act_rte["trip_status"] =="A" ) {
                ...
                if ( $m_status == 0 ) {
                    ...
                    $last_sent_order = $wr_dcd_countdown["sequence"];
                }
            }
...
function get_countdown_values($in_dcd_param)
{
    ...
    else
    {
		...
		$etd_last_sent_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["etd_last_sent"]);
        $eta_last_sent_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["eta_last_sent"]);
        $tmpval = $etd_last_sent_datetime;
        if ($tmpval == NULL)
            $tmpval = $eta_last_sent_datetime;
        if ($tmpval == NULL)
        {
//            error("500", "get_countdown_values: eta_last_sent and etd_last_sent are both invalid");
//            die;
            $tmpval = DateTime::createFromFormat('Y-m-d H:i:s', "1970-01-01 00:00:00");
        }
...
function should_show($in_dcd_param)
{
    ...
    $etd_last_sent_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["etd_last_sent"]);
	$eta_last_sent_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["eta_last_sent"]);
	$rtpi_etd_sent_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["rtpi_etd_sent"]);
    ...
    // Is arrival time is within display window or has passed
    // --------------------------------------------------------------------
    if (!$rtpi_etd_sent_datetime)
        return false;

    $m_comp_int = $rtpi_etd_sent_datetime->diff($curr_time);
...
function get_countdown($loccode, $send_mode, $in_dcd_param)
{
    ...
    if ( should_show ($in_dcd_param) ) {
        ...
        (
            "route_code" => trim($m_dcd_countdown["route_code"]),
            "service_description" => trim($m_dcd_countdown["service_description"]),
            "destination1" => trim($m_dcd_countdown["destination_text"]), 
            "destination2" => trim($m_dcd_countdown["destination_text"]),
            "eta" => trim($m_dcd_countdown["display_mins"]),
            "pub" => trim($m_dcd_countdown["output_pub_hhmm"]),
            "time" => trim($m_dcd_countdown["countdown_time"]),
            "real_time_eta" => trim($m_dcd_countdown["rtpi_eta_sent"]),
            "real_time_etd" => trim($m_dcd_countdown["rtpi_etd_sent"]),
            "scheduled_time_eta" => trim($m_dcd_countdown["pub_eta_sent"]),
            "scheduled_time_etd" => trim($m_dcd_countdown["pub_etd_sent"]),
            "vehicle_code" => trim($wr_vehicle["vehicle_code"]),
            "gps_time" => trim($m_dcd_countdown["gps_time"]),
            "gpslat" => trim($m_dcd_countdown["gpslat"]),
            "gpslong" => trim($m_dcd_countdown["gpslong"]),
            "wheelchair_access" => trim($m_dcd_countdown["wheelchair_access"])
        );
    }
    return $l_counter;
...
function get_countdown_for_display($in_dcd_param)
{
    ...
    $m_dcd_countdown["route_code"] = $wr_route["route_code"];
    $m_dcd_countdown["service_description"] = $wr_service["description"];
    ...
    $m_dcd_countdown["pub_eta_sent"] = $wr_dcd_countdown["pub_eta_sent"];
    $m_dcd_countdown["pub_etd_sent"] = $wr_dcd_countdown["pub_etd_sent"];
    $m_dcd_countdown["rtpi_eta_sent"] = $wr_dcd_countdown["rtpi_eta_sent"];
    $m_dcd_countdown["rtpi_etd_sent"] = $wr_dcd_countdown["rtpi_etd_sent"];
    ...
    $sql =
                    ...
                    " AND service_patt.rpat_orderby = ". $wr_dcd_countdown["sequence"]. 
    ...
    $m_dcd_countdown["gps_time"] = $wr_unit_status["gps_time"];
    $m_dcd_countdown["gpslat"] = $wr_unit_status["gpslat"];
    $m_dcd_countdown["gpslong"] = $wr_unit_status["gpslong"];
       ...
       while(is_array($row))
       {
            ...
            if ( $l_dcd_param["day_of_week"]) {
                ...
                if ( get_current_time()->format("w") != $l_dcd_param["day_of_week"] ) {
                    //echo "IGNORING DOW SPECIFIER ". $l_dcd_param["day_of_week"]. " vs ". get_current_time()->format("w"); 
                    $row = fetchPDO ($rid, "NEXT");
                    continue;
                }
            }
...
function format_web_stop_data($in_dcd_param)
{
    ...
    $w_service_id         = 0;
    $w_service_desc       = "";
    $wr_timetable_journey_live         = array();
    $w_location_id        = 0;
    $l_vehicle_code       = "";
    $est_dep_arr          = "";
    $pub_time             = "";
    ...
    {
        $m_dcd_countdown["display_mins"] = $display_mins. " min";
        if ($wait_time_all_secs < 30)
        {
        ...
        if ($est_hr_dt == NULL)
        {
            // We don't have a published time for non-timing-point stops on AUT
            // trips, so use the profile-generated "scheduled time" which would
            // have been stored in rtpi_etd_sent.
            $est_hr_dt = DateTime::createFromFormat('Y-m-d H:i:s', $wr_dcd_countdown["rtpi_etd_sent"]);
...
function get_location_details($in_loc)
{
    ...
    $sql = "SELECT location.description, atco_code, naptan_code
        FROM location, outer naptan_stop_point
        WHERE naptan_stop_point.atco_code = location.location_code
    ...
    $wr_countdowns["location"] = array(
            "description" => trim($wr_location["description"]),
            "atco" => trim($w_atco_code), 
            "naptan" => trim($w_naptan_code),
...
function webstop_display_json()
{
    ...
    if (!$wr_countdowns["location"]["atco"])
    {
        error("404", "Unknown Location", "json");
        return;
    }

    header('Content-Type: application/json');
    echo '{"MonitoredLocation":';
    echo '{';
    echo '"Timestamp":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '",';
    echo '"Id":"' . $wr_countdowns["location"]["atco"] . '",';
    echo '"Naptan":"' . $wr_countdowns["location"]["naptan"] . '",';
    echo '"Name":"' . $wr_countdowns["location"]["description"] . '"';
    ...
    foreach ($wr_countdowns["arrivals"] as $k => $v)
    {
        ...
        if ($duein != "D")
        {
            ...
            if ($ct > 0)
                echo ',';
            echo '{';
            echo '"Service":"' . $v["route_code"] . '",';
            echo '"PublishedServiceName":"' . $v["service_description"] . '",';
            echo '"Destination":"' . $v["destination1"] . '",';

            $millis = strtotime($v["scheduled_time_eta"]);
            if ($millis == 0 && $vehcd == "AUT")
            {
                $millis = strtotime($v["real_time_eta"]);
                echo '"ScheduledArrival":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '",';
            }
            else
                echo '"ScheduledArrival":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '",';

            $millis = strtotime($v["scheduled_time_etd"]);
            if ($millis == 0 && $vehcd == "AUT")
            {
                $millis = strtotime($v["real_time_etd"]);
                echo '"ScheduledDeparture":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '"';
            }
            else
                echo '"ScheduledDeparture":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '"';
            ...
            if ($duein != "P" && $vehcd != "AUT")
            {
                echo ',"MonitoredCall":{';
                /*
                echo "<VehicleCode>$vehcd</VehicleCode>";
                echo "<VehicleLocation>";
                $millis = strtotime($v["gps_time"]);
                echo('<RecordedAtTime>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</RecordedAtTime>');
                echo "<Latitude>" . $v["gpslat"] . "</Latitude>";
                echo "<Longitude>" . $v["gpslong"] . "</Longitude>";
                echo "</VehicleLocation>";
                */
                if ($v["wheelchair_access"] == 1)
                    echo '"WheelchairAccess":"true",';
                $millis = strtotime($v["real_time_eta"]);
                echo '"ExpectedArrival":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '",'; 
                $millis = strtotime($v["real_time_etd"]);
                echo '"ExpectedDeparture":"' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '",';
                echo '"DisplayTime":"' . $duein . '"}';
            }
            echo '}';
            $ct++;
        }
...
function webstop_display_xml()
{
    ...
    echo '<?xml version="1.0"?' . '>'; // vim syntax highlighting
    echo "<MonitoredLocation>";
    echo '<Timestamp>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</Timestamp>'; 
    echo "  <Id>".$wr_countdowns["location"]["atco"]."</Id>";
    echo "  <Naptan>".$wr_countdowns["location"]["naptan"]."</Naptan>";
    echo "  <Name>".$wr_countdowns["location"]["description"]."</Name>";
    ...
    foreach ($wr_countdowns["arrivals"] as $k => $v)
    {
        ...
        if ($duein != "D")
        {
            ...
            echo "<Call>";
            echo "<Service>" . $v["route_code"] . "</Service>";
            echo "<PublishedServiceName>" . $v["service_description"] . "</PublishedServiceName>";
            echo "<Destination>" . $v["destination1"] . "</Destination>";

            $millis = strtotime($v["scheduled_time_eta"]);
            if ($millis == 0 && $vehcd == "AUT")
            {
                $millis = strtotime($v["real_time_eta"]);
                echo '<ScheduledArrival>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ScheduledArrival>'; 
            }
            else
                echo '<ScheduledArrival>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ScheduledArrival>'; 
            ...
            {
                $millis = strtotime($v["real_time_etd"]);
                echo '<ScheduledDeparture>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ScheduledDeparture>'; 
            }
            else
                echo '<ScheduledDeparture>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ScheduledDeparture>'; 
            ...
            if ($duein != "P" && $vehcd != "AUT")
            {
                ...
                */
                if ($v["wheelchair_access"] == 1)
                    echo "<WheelchairAccess>true</WheelchairAccess>";
                $millis = strtotime($v["real_time_eta"]);
                echo('<ExpectedArrival>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ExpectedArrival>'); 
                $millis = strtotime($v["real_time_etd"]);
                echo('<ExpectedDeparture>' . date('Y-m-d\TH:i:s', $millis) . $offsetString . '</ExpectedDeparture>'); 
                echo "<DisplayTime>$duein</DisplayTime>";
                echo "</MonitoredCall>";
            }
            echo "</Call>";
            $ct++;
    ...
    foreach ($wr_countdowns["arrivals"] as $k => $v)
    {
        ...
        if ($duein != "D")
        {
            ...
            if ($ct == 0)
                echo "<calls>";
            echo "<call>";
            echo "<route>$service_description</route>";
            echo "<destination>" . $v["destination1"] . "</destination>";
            echo "<eta>$duein</eta>";
            echo "</call>";
            $ct++;
...
<head>
    ...
    <meta http-equiv=refresh content="30; url="index.php?r=webstop/popout&locations='.$wr_countdowns["location"]["atco"].'">
    <title>Connexionz UK NetView Demo</title>
    <script src="/staffs/iconnex/js/pwi.js" type="text/javascript"></script>
    <link rel="stylesheet" type="text/css" href="/staffs/iconnex/css/pwi.css" />
</head>
<body class="theBody">
<div id="header">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td align="center" class="headerTop"><table width="830" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="444" align="left" valign="top"><img src="/staffs/iconnex/images/header.jpg" alt="connexionz logo" width="444" height="93" /></td>
              <td align="left" valign="top"><img src="/staffs/iconnex/images/header-image-home.jpg" width="310" height="93" alt="" /></td>
            </tr>
          </table></td>
      </tr>
    </table>
...
';
    ...
    if (count($wr_countdowns["arrivals"]) > 0)
        echo "<TR><TH align=left>Route</TH><TH align=left>Destination</TH><TH align=right>Due</TH></TR>";
    ...
    foreach ( $wr_countdowns["arrivals"] as $k => $v )
    {
        ...
        if ($duein != "D")
        {
            ...
            echo "<TR><TD align=left>$route</TD><TD>" . $v["destination1"] . "</TD><TD ALIGN=right>$duein</TD></TR>";
            $ct++;
        }
...
function messages($format = "xml")
{
    ...
    // Parse parameters
    if ( !($stop = get_request_item("locations")) )
    {
        echo "Parameters:  location,location stop|admin show_veh[0|1] show_auts[0|1]";
        die;
    }
...
and CURRENT between dcd_message_loc.display_time and dcd_message_loc.expiry_time";
    ...
    if ($format == "json")
    {
        while (true)
        {
            ...
            if ($i > 0)
                echo ',';
            echo '{"Message":"' . trim($row["message_text"]) . '"}';
    ...
    else
    {
        while (true)
        {
            ...
            if ($i++ == 0)
                echo "<Messages>\n";
            echo "  <Message>" . trim($row["message_text"]) . "</Message>\n";
        }
...
function error($code, $msg = NULL, $format = "xml")
{
    if ($code == "500")
