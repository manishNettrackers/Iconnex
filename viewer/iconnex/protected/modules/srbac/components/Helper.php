...
//      $len = strlen($cleverName);
  ...
  private static function _install($demo) {
    ...
    try {
      ...
      //create tables
      //$db->createCommand("drop table " . $itemTable . ";")->execute();
      ...
      $db->createCommand("drop table " . $itemChildTable . ";")->execute();
      $sql = "create table " . $itemChildTable . " (parent varchar(64) not null,
                                              child varchar(64) not null,
                                              primary key (parent,child)
                                              );";
      $db->createCommand($sql)->execute();
      $sql = "alter table " . $itemChildTable . " add constraint foreign key (parent) references " . $itemTable . " (name) on delete cascade constraint authpar1";
      $db->createCommand($sql)->execute();
      $sql = "alter table " . $itemChildTable . " add constraint foreign key (child) references " . $itemTable . " (name) on delete cascade constraint authchld1";
      $db->createCommand("drop table " . $assignmentTable . ";")->execute();
      $db->createCommand($sql)->execute();
      ...
      $db->createCommand($sql)->execute();
      $sql = "alter table " . $assignmentTable . " add constraint foreign key (itemname) references " . $itemTable . " (name) on delete cascade constraint authassg1";
      $db->createCommand($sql)->execute();
      //Insert Authorizer
