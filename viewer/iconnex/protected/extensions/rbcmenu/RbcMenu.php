...
Yii::import('application.extensions.rbcmenu.Browser');

class RbcMenu extends CMenu
{
    private $baseUrl;
    private $nljs;
    ...
    */
    public function registerClientScripts()
    {
        ...
        $cs->registerScript('rbcmenu_'.$this->getId(), $js, CClientScript::POS_READY);
    }
 	  ...
 	  public function registerCssFile($url=null)
	  {
        // add the css
        if ($this->baseUrl === '')
            throw new CException(Yii::t('RbcMenu', 'baseUrl must be set. This is done automatically by calling publishAssets()'));
 
	  	  $cs=Yii::app()->getClientScript();
	  	  if($url===null) { 
	  		  $url=$this->baseUrl.'/rbcmenu.css';

          $cs->registerCssFile($url,'screen');
          $browser = Browser::detect();
          if ($browser['name'] == 'msie' && $browser['version'] < 8)
            $cs->registerCssFile($this->baseUrl.'/rbcmenu_iestyles.css','screen');
        } else {
	  	    $cs->registerCssFile($url,'screen');
        }
	  }
    ...
    */
    public function run()
    {
          ...
          $htmlOptions['id']='tm-nav-container';
          echo CHtml::openTag('div',$htmlOptions)."\n";          
          $htmlOptions['id']='tm-nav-bar';
          echo CHtml::openTag('div',$htmlOptions)."\n";
          parent::run();
