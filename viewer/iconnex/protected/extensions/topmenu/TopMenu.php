...
Yii::import('application.extensions.topmenu.Browser');
...
class TopMenu extends CMenu
{
    ...
    */
    public function registerClientScripts()
    {
        ...
        $cs->registerScript('topmenu_'.$this->getId(), $js, CClientScript::POS_READY);
    }
 	  ...
 	  public function registerCssFile($url=null)
	  {
        // add the css
        if ($this->baseUrl === '')
            throw new CException(Yii::t('TopMenu', 'baseUrl must be set. This is done automatically by calling publishAssets()'));
 
	  	  $cs=Yii::app()->getClientScript();
	  	  if($url===null) { 
	  		  $url=$this->baseUrl.'/topmenu.css';
          $cs->registerCssFile($url,'screen');
          $browser = Browser::detect();
          if ($browser['name'] == 'msie' && $browser['version'] < 8)
            $cs->registerCssFile($this->baseUrl.'/topmenu_iestyles.css','screen');
        } else {
	  	    $cs->registerCssFile($url,'screen');
        }
	  }
