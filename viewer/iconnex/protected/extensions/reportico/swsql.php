 ...
 * @package Reportico
 * @version $Id: swsql.php,v 1.10 2012-08-25 14:05:03 peter Exp $
 */
...
class reportico_sql_parser
{
	...
	function parse()
	{
        ...
        while ( $ptr < strlen($sql) )
        {
            ...
            else if ( preg_match("/\sFROM\s/", $bit6 ) )
            {
        ...
        else
        {
			...
			if ( $col )
			{
				$this->parse_column_list($this->sql_raw, $col, $selpos );
    ...
    // alias is added in
	// ----------------------------------------------------------------------------
	function parse_column_list( &$original_sql, $in_string, $offset_in_original_sql )
	{
		...
		foreach ( $collist as $k => $colitem )
		{
				...
				if ( !$this->parse_column($k + 1, trim($colitem), $auto_gen_alias ) )
					return false;
	...
	// ----------------------------------------------------------------------------
	function parse_column( $in_colno, $in_string, &$auto_gen_alias )
	{
		$err = false;
