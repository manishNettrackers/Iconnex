...
$debug = false;
    ...
    if (!$iconnex->executeSQL("SET ISOLATION TO DIRTY READ")) return;
...
if ( $fetchOk )
{
$sql = "SELECT t_days.day, ' ' dtime, start_time, " .
    " publish_tt.duty_no, " .
    " publish_tt.trip_no, route_code, service_code, t_route.route_id, " .
    " operator.operator_code operator_code, service.service_id, " .
    " publish_tt.pub_ttb_id pub_ttb_id, publish_tt.runningno runningno, " .
    " event.event_code event_code, event.event_id, notes[1,1] over_midnight, operator.operator_id " .
    " FROM operator,route_for_user t_route,service, " .
    " t_days, publish_tt,event_pattern, event " .
    " WHERE 1 = 1 " .
    ...
    " AND day between wef_date and wet_date " .
    " and " .
    " ( " .
    "     ( weekday(t_days.day) between rpdy_start and rpdy_end " .
    "         and notes[1,1] = '0' " .
    " ) " .
    "         or " .
    "     ( weekday(t_days.day + 1) between rpdy_start and rpdy_end " .
    "         and notes[1,1] = '1' ) " .
    " ) " .
    " AND start_time between '00:00:00' and '23:59:59' ";

$sql .= " AND start_time between $fromtime and $totime";
...
if ($rbd)
{
    $sql .= " AND runningno MATCHES $rbd";
}

if ($dty)
{
    $sql .= " AND duty_no MATCHES $dty";
}
;
...
$sql = "SELECT active_status, scheduled_start , t_timetable.pub_ttb_id, t_timetable.day, over_midnight, t_timetable.operator_code," .
    ...
    " from t_timetable, t_ttb_status, " . $cancelledLink . " tt_mod, " . $nexttripLink . " publish_tt next_pub, " . $allocvehLink . " t_allocveh, " . $diversionLink . " t_diversions, outer t_actdet, $latenessLink t_trips " . 
    ...
    " and t_ttb_status.pub_ttb_id = tt_mod.pub_ttb_id" .
...
for ($ct = 0; $ct < count($this->columns); $ct++)
{
   ...
   if ( $col->query_name == "lateness_min" ) $col->attributes["column_display"] = "hide";
   if ( $col->query_name == "xlabel" ) $col->attributes["column_display"] = "hide";
