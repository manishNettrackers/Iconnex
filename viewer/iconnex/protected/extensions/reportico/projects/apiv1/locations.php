...
// Work around bug in reportico lookups
if ($crt == " AND 1 = 0")
    $sql .= " AND route.route_id IN (-1)";
else
{
    if ($crt)
        $sql .= " AND route.route_id IN ($crt)";
}

if ($cop)
    $sql .= " AND route.operator_id IN ($cop)";
...
WHERE 1 = 1  AND l.route_area_id = ra.route_area_id
  ...
  and s.atco_code = l.location_code";
...
$_connection->Execute($sql) or
      print $sql."<BR>".$_connection->ErrorMsg()."<BR>";
