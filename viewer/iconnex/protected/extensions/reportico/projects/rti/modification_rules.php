...
function custom_project_modifications($pdo, &$return_status)
{
        ...
        {
            case "destination":
                custom_mod_destination($iconnex,$return_status);
                break;
...
function custom_mod_destination ($pdo, &$status)
{
    $oper = get_request_item("oper", "");
	...
	{
        $status["errstat"] = 400;
        $status["msgtext"] = "No destination specified";
        return;
	}
    ...
    $dest_code = get_request_item("dest_code");
    $long_name = get_request_item("long_name");
    $short_name = get_request_item("sign_30_char");

	IF ( $keyid )
	{
		$sql = "UPDATE destination SET ( dest_long, dest_short1 ) = ( '".$long_name."', '".$short_name."' ) WHERE dest_id = $keyid";
	}
    //showStuff($status,$sql);
...
function showStuff(&$status, $txt = false)
{
	$str = "";
	foreach ( $_REQUEST as $k => $y )
	{
	$str .= "$k = $y\n";
    	...
    	$keyid = "ee";
    $status["errstat"] = 567;
    $status["msgtext"] = "Oh my rod there is an error1<BR>$str <BR>$txt";

    $fd = fopen("/tmp/fred7", "w+" );
    fwrite($fd, "oooo".$txt);
    fwrite($fd, $str);
    fclose($fd);
}
