 ...
 * @package Reportico
 * @version $Id: swutil.php,v 1.28 2012-08-21 22:19:16 root Exp $
 */
...
function set_up_reportico_session()
{
	...
	if ( !$session_name || isset($_REQUEST['clear_session']))
	{
		...
		if (isset($_REQUEST['clear_session'])) 
		{
			...
			unset($_SESSION["target_menu"]);
			unset($_SESSION["latestRequest"]);
			unset($_SESSION["linkbaseurl"]);
			unset($_SESSION["linkbaseurl"]);
            unset($_SESSION["execute_mode"]);
            unset($_SESSION["project"]);
            unset($_SESSION["target_menu"]);
            unset($_SESSION["language"]);
            unset($_SESSION["firstTimeIn"]);
            unset($_SESSION["latestRequest"]);
            unset($_SESSION["template"]);
            unset($_SESSION["forward_url_get_parameters"]);
            unset($_SESSION["linkbaseurl"]);
            ...
            unset($_SESSION["progress_text"]);
            unset($_SESSION["progress_status"]);
            unset($_SESSION["target_format"]);
            unset($_SESSION["xmlin"]);
            unset($_SESSION["xmlout"]);
            unset($_SESSION["session_user"]);
            unset($_SESSION["target_show_detail"]);
            unset($_SESSION["target_show_graph"]);
            unset($_SESSION["target_show_group_headers"]);
            unset($_SESSION["target_show_group_trailers"]);
            unset($_SESSION["target_show_column_headers"]);
            unset($_SESSION["target_show_criteria"]);
		}
	}
	else
	{
...
function parse_date($in_keyword, $in_time = false, $in_mask = "%d/%m/%Y" )
{
	...
	switch ( $in_keyword )
	{
        ...
        case "STARTOFLASTWEEK":
            ...
            $new_time = strtotime("last Sunday", $new_time);
            break;
        ...
        case "MONOFLASTWEEK":
            $new_time = strtotime("last Friday");
            $new_time = strtotime("last Monday", $new_time);
            break;
...
/*
    Searches project config.php parameters for a particular parameter
    relating to the specified report
*/
function derive_config_report_params ($report_name, $parameter, $default)
{
    $param = "SW_CONFIG_".$report_name."_".$parameter;
...
function convertTimeToXMLTimeWithOffset($datetime)
{
    if (date_default_timezone_get() == 'UTC')
        return date_format(date_create_from_format('Y-m-d H:i:s', $datetime), 'Y-m-d\TH:i:s\Z');
    else
        return date_format(date_create_from_format('Y-m-d H:i:s', $datetime), 'Y-m-d\TH:i:sP');
}
