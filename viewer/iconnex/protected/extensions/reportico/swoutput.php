 ...
 * @package Reportico
 * @version $Id: swoutput.php,v 1.20 2012-08-30 18:25:32 root Exp $
 */
...
class reportico_report extends reportico_object
{
	...
	{
		if ( $_SESSION["target_show_criteria"] )
		{
	...
	function each_line($val)
	{
		...
		if ( $_SESSION["target_show_group_trailers"] )
		    $this->after_group_trailers();
	...
	function before_group_headers()
	{
	    ...
	    if ( $_SESSION["target_show_group_headers"] )
...
class reportico_report_pdf extends reportico_report
{
    ...
    function draw_cell($w,$h=0,$txt='',$implied_styles="PBF",$ln=0,$align='',$valign="T", $link='')
    {
        ...
        if ( $link )
        {
            if ( !preg_match("/^http:\/\//", $link) && !preg_match("/^\//", $link ) )
                $link = SW_HTTP_PATH_TO_REPORTICO.$link;
            if ( preg_match("/^\//", $link ) )
	...
	{
		if ( !$_SESSION["target_show_column_headers"] )
			return;
...
//var_dump($value);
	...
	function each_line($val) // PDF
	{
		...
		$this->yjump = 0;
		if ( $this->body_display == "show" && $_SESSION["target_show_detail"] )
		{
...
class reportico_report_html_template extends reportico_report
{
	...
	function finish ()
	{
		...
		if ( $this->line_count < 1 )
		{
			...
			if ( $forward )
				$forward .= "&";
			$this->text .= '<div class="swRepBackBox"><a class="swLinkMenu" href="'.session_request_item('linkbaseurl', SW_USESELF).'?'.$forward.'execute_mode=PREPARE&session_name='.session_id().'">'.template_xlate("GO_BACK").'</a></div>';
	...
	function format_column_header(& $column_item)	//HTML
	{
	    ...
	    $this->text .= '<TD " '.$this->get_style_tags($colstyles, $this->query->output_header_styles).'>';
        $this->text .= $padstring;
        $this->text .= "</TD>";
	}
    ...
    function format_images ( $image )
    {
        ...
        if ( isset($image["height"]) && $image["height"] ) $txt .= ' height="42"';
        if ( isset($image["width"]) && $image["width"] ) $txt .= ' width="42"'; 
        $txt .= " /> ";
        return $txt;
    }
    ...
    function format_hyperlinks ( $hyperlinks, $padstring )
    {
        ...
        if ( isset($hyperlinks["styles"]) )
            $styles = $hyperlinks["styles"];

        if ( $hyperlinks["label"] == "<SELF>" )
        {
            $txt = '<a '.$styles.' href="'.$hyperlinks["url"].'"'.$open.'>'.$padstring.'</a>';
        }
        else
            $txt = '<a '.$styles.' href="'.$hyperlinks["url"].'"'.$open.'>'.$hyperlinks["label"].'</a>';
        return $txt;
    }
	...
	function format_headers()
	{
		if ( $this->body_display != "show" )
			return;
		$this->text .="<tr class='swRepColHdrRow'>";
		foreach ( $this->query->display_order_set["column"] as $w )
			$this->format_column_header($w);
		$this->text .="</tr>";
	}
	...
	function plot_graph(&$graph)
	{
		...
		$this->text .= '<TR>';
		$this->text .= '<TD class="swRepResultGraph" width = "80%" colspan="60">';
		if ( $url_string )
		{
...
class reportico_report_json extends reportico_report
{
    ...
    var $report_name;
    var $datalabel = "data";
    var $datalinelabel = "dataline";
	
	function reportico_report_json ()
	{
		$this->page_width = 595;
		$this->page_height = 842;
		$this->column_spacing = "2%";
	...
	function start ()
	{
        ...
        $this->datalabel = derive_config_report_params($this->report_name, "datalabel", "data");
        $this->datalinelabel = derive_config_report_params($this->report_name, "datalinelabel", "dataline");
		$this->results=array(
			"title" => $title,
			"displaylike" => array(),
			$this->datalabel => array()
			);
	...
	function finish ()
	{
        ...
        global $g_project;
        // structure the results into
        $usesmartydir = false;
        $testdir =  find_best_location_in_include_path( "projects/".$g_project );
        if ( $testdir )
        {
            if ( is_file ( $testdir."/json.tpl" ) )
                $usesmartydir = $testdir;
        }
        else
        {
            $testdir =  find_best_location_in_include_path( "templates" );
            if ( $testdir )
            {
                if ( is_file ( $testdir."/json.tpl" ) )
                    $usesmartydir = $testdir;
        ...
        {
		    $json = json_encode($this->results["data"]);

            $smarty = new smarty();
            $smarty->template_dir = find_best_location_in_include_path( $usesmartydir);
            ...
            {
                $keysearch = "_" . $this->report_name . "_apiheader_";
                if ($keyPos = strrpos($key, $keysearch))
                {
                    $headerConstant = substr($key, $keyPos + strlen($keysearch));
                    $apiheader[$headerConstant] = $value;
                }
            }
            $smarty->assign('apiheader', $apiheader);
            if ($json)
                $smarty->assign('json', $json);
            $smarty->assign('datalabel', $this->datalabel);
            $data = $smarty->fetch('json.tpl');
        }

		header('Cache-Control: no-cache, must-revalidate');
		header('Content-Type: application/json');
		header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
		header('Content-Disposition: inline; filename='.$this->reporttitle.'.json');
	...
	function each_line($val)
	{
        ...
        if ( $this->line_ct == 0 )
        {
                ...
                {
                    $now = new DateTime();
                    $qn->column_value .= ",Timestamp=".$now->format("Y-m-d H:i:s");
                    $arr = explode ( ",", $qn->column_value );
                    foreach ( $arr as $k => $v )
                    {
                        $gval = preg_replace("/^.*=/U", "", $v);
                        $gkey = preg_replace("/=.*/", "", $v);

                        //$arr1 = explode ( "=", $v );
                        //$this->results["displaylike"][$arr1[0]] = $arr1[1];
                        $this->results["displaylike"][$gkey] = $gval;
                    }
                }
        }
...
class reportico_report_xml extends reportico_report
{
	...
	function finish ()
	{
        ...
        else
        {
            ...
            $xml = $this->arrayToXML($this->results[$this->datalabel], new SimpleXMLElement('<'.$this->datalabel.'/>'))->asXML();
            $xml = preg_replace('/^.+\n/', '', $xml);
            $len = strlen($xml);
		...
		header("Content-Length: $len");
		header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
		header('Content-Disposition: inline; filename='.$this->reporttitle.'.xml');
    ...
    /*
    * Converts Reportico output held as hierarchical array in to corresponding XML 
    */
	...
	function each_line($val)
	{
		...
		$this->results[$this->datalabel][$linekey] = array();
       	...
       	if ( $this->line_ct == 0 )
       	{
               ...
               {
                   $now = new DateTime();
                   $qn .= ",Timestamp=".$now->format("Y-m-d H:i:s");
                   $arr = explode ( ",", $qn->column_value );
                   foreach ( $arr as $k => $v )
                   {
                       $arr1 = explode ( "=", $v );
		...
		foreach ( $this->query->display_order_set["column"] as $col )
	  	{
			...
			$this->results[$this->datalabel][$linekey][preg_replace("/ /", "", $coltitle)] = $qn->column_value;
  		}
		$this->line_ct++;
	}
}
...
class reportico_report_jquerygrid extends reportico_report
{
	...
	function reportico_report_jquerybuilder ()
	{
		$this->page_width = 595;
		$this->page_height = 842;
		$this->column_spacing = "2%";
	...
	function start ()
	{
		...
		$ct=0;
		$this->report_name =  preg_replace("/\.xml$/", "", $this->query->xmlinput);
	    $this->row_checkboxes = derive_jquerygrid_rep_params ( $this->report_name, "checkboxes", false );
		if ( $this->row_checkboxes )
		{
			$this->gridparams["multiselect"] = true;
		}
	    $this->key_column = derive_jquerygrid_rep_params ( $this->report_name, "primary_key", false );

        // Identify dynamic buttons
        $buttonct = 1;
        while ( $button = derive_jquerygrid_rep_params ( $this->report_name, "custom_button".$buttonct, false ) )
        {
            $this->buttons[$button] = $button;
            //$this->buttonids[$button] = derive_jquerygrid_rep_params ( $this->report_name, "custom_button".$buttonct."_ids", false ) );
            $buttonct++;
        }

        
        $this->graphopt["xlabelcol"] = derive_jquerygrid_rep_params ( $this->report_name, "graph_xlabelcol", false );
        $this->graphopt["plots"] = array();
        $plotct = 1;

        while ( $plot = derive_jquerygrid_rep_params ( $this->report_name, "graph_plotcol".$plotct, false ) )
        {
            $plottype = derive_jquerygrid_rep_params ( $this->report_name, "graph_plottype".$plotct, "bar" );
            $legend = derive_jquerygrid_rep_params ( $this->report_name, "graph_legend".$plotct, "legend" );
            $this->graphopt["plots"][] = array(
                            "plotcol" => $plot,
                            "plottype" => $plottype,
                            "legend" => $legend
	...
	function finish ()
	{
		...
		$retarr["minihide"] = $this->minihide;
		$retarr["graphopt"] = $this->graphopt;
		$retarr["buttons"] = $this->buttons;
		$retarr["timestamp"] = $this->timestamp;
		$retarr["gridparams"] = $this->gridparams;
		...
		header("Content-Length: $len");
		header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
		header('Content-Disposition: inline; filename=reportico.json');
	...
	function each_line($val)
	{
        ...
        foreach ( $this->query->groups as $name => $group)
		{
			...
			foreach ($group->headers as $gphk => $col )
			{
                ...
                {
                    echo "cmp $col->query_name == $this->key_column <BR>";
			    ...
			    if ( $this->line_ct == 0 )
			    {
				    ...
				    if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "minihide", "" ) )
					    $this->minihide[] = $col->query_name;
					...
					if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "stype", "" ) )
						$this->colmodel[count($this->colmodel)-1]["stype"] = $v;
                    if ( $v == 'text' ) // Default string search columns to string type conditons
						$this->colmodel[count($this->colmodel)-1]["searchoptions"]["sopt"] = array('eq', 'ne', 'bw', 'bn', 'ew', 'en', 'cn', 'nc', 'nu', 'nn', 'in', 'ni');
					if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "searchtype", "" ) )
						$this->colmodel[count($this->colmodel)-1]["searchtype"] = $v;
				    ...
				    if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "filtertype", "" ) )
                    {
                        if ( $v == "select" ) // Generate filter options based on data in columns
                        {
                            ...
                            if ( ! $this->colmodel[count($this->colmodel)-1]["editoptions"]  )
                            {
                                ...
                                if ( $url = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "dataurl", "" ) )
                                {
                                    $this->colmodel[count($this->colmodel)-1]["editoptions"]["dataUrl"] = $url;
                                    $this->colmodel[count($this->colmodel)-1]["editoptions"]["size"] = 1;
                                }
                                else
                                {
                                    $this->colfilters[$col->query_name] = array();
                ...
                if ( $this->colmodel[$this->colmap[$col->query_name]]["stype"] == "select" && isset($this->colfilters[$col->query_name]) )
                {
		...
		foreach ( $this->query->display_order_set["column"] as $col )
	  	{
			...
			if ( $this->line_ct == 0 )
			{
				...
				if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "stype", "" ) )
					$this->colmodel[count($this->colmodel)-1]["stype"] = $v;
                if ( $v == 'text' ) // Default string search fields to string type search conditions
					$this->colmodel[count($this->colmodel)-1]["searchoptions"]["sopt"] = array('eq', 'ne', 'bw', 'bn', 'ew', 'en', 'cn', 'nc', 'nu', 'nn', 'in', 'ni');
				if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "searchtype", "" ) )
					$this->colmodel[count($this->colmodel)-1]["searchtype"] = $v;
				if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "filtertype", "" ) )
                {
                    ...
                    else if ( $v == "readonly" ) // Generate filter options based on data in columns
                    {
                        if ( ! $this->colmodel[count($this->colmodel)-1]["editoptions"]  )
                        {
                            $this->colmodel[count($this->colmodel)-1]["editoptions"] = array();
                        }
                        $this->colmodel[count($this->colmodel)-1]["editoptions"]["readonly"] = "readonly";
                    }
                }
				if ( $v = derive_jquerygrid_col_params ( $this->report_name, $col->query_name, "editoptions", "" ) )
					$this->colmodel[count($this->colmodel)-1]["editoptions"] = $v;
            ...
            {
                //if ( !array_key_exists( $qn->column_value, $this->colfilters[$col->query_name] ) )
                if ( isset($this->colfilters[$col->query_name]) &&  !array_key_exists( $qn->column_value, $this->colfilters[$col->query_name] ) )
                {
                    //$this->colmodel[$this->colmap[$col->query_name]]["editoptions"][$qn->column_value] = $qn->column_value;
                    $this->colfilters[$col->query_name][$qn->column_value] = "1";
                    $this->colmodel[$this->colmap[$col->query_name]]["editoptions"]["value"] .= ";".$qn->column_value.":".$qn->column_value;
            ...
            if ( $qn->output_hyperlinks )
		        $this->results[count($this->results)-1]["cell"][] = $this->format_hyperlinks($qn->output_hyperlinks, $qn->column_value);
            else
		        $this->results[count($this->results)-1]["cell"][] = $qn->column_value;
