 ...
 * @package Reportico
 * @version $Id: reportico.php,v 1.41 2012-08-24 23:18:42 peter Exp $
 */
...
global $g_no_sql;
global $g_framework_parent;   // Links to framework reportico is embedded into
global $framework_user;   // Contains user id relating to databases if relevant
...
class reportico extends reportico_object
{
	...
	function reportico($in_framework = false, $in_name = "swquery")
	{
		reportico_object::reportico_object();

        global $g_framework_parent;
		$this->name = $in_name;
		$this->parent_query =& $this;

        if (  $in_framework )
		    $g_framework_parent = $in_framework;
	}
	...
	function &create_graph()
	{
		...
		$graph = new reportico_graph("internal");
		$this->graphs[] =& $graph;
		return $graph;
	}
	...
	function set_request_columns()
	{
	    ...
	    $this->target_show_detail = session_request_item("target_show_detail", true, !isset($_SESSION["target_show_detail"]));
	    $this->target_show_graph = session_request_item("target_show_graph", true, !isset($_SESSION["target_show_graph"]));
	    $this->target_show_group_headers = session_request_item("target_show_group_headers", true, !isset($_SESSION["target_show_group_headers"]));
	    $this->target_show_group_trailers = session_request_item("target_show_group_trailers", true, !isset($_SESSION["target_show_group_trailers"]));
	    $this->target_show_column_headers = session_request_item("target_show_column_headers", true, !isset($_SESSION["target_show_column_headers"]));
	    $this->target_show_criteria = session_request_item("target_show_criteria", true, !isset($_SESSION["target_show_criteria"]));
		if ( $_SESSION["firstTimeIn"] )
        {
	        ...
	        $_SESSION["target_show_detail"] = true;
	        $_SESSION["target_show_graph"] = true;
	        $_SESSION["target_show_group_headers"] = true;
	        $_SESSION["target_show_group_trailers"] = true;
	        $_SESSION["target_show_column_headers"] = true;
	        $_SESSION["target_show_criteria"] = false;
        }
        else
        {
            ...
            if ( $this->execute_mode == "EXECUTE" )
            {
	            ...
	            $_SESSION["target_show_detail"] = $this->target_show_detail;
	            $_SESSION["target_show_graph"] = $this->target_show_graph;
	            $_SESSION["target_show_group_headers"] = $this->target_show_group_headers;
	            $_SESSION["target_show_group_trailers"] = $this->target_show_group_trailers;
	            $_SESSION["target_show_column_headers"] = $this->target_show_column_headers;
	            $_SESSION["target_show_criteria"] = $this->target_show_criteria;
            }
        }
        if ( isset ( $_REQUEST["target_show_detail"] ))  $_SESSION["target_show_detail"] = $_REQUEST["target_show_detail"];
        if ( isset ( $_REQUEST["target_show_detail"] ))  $_SESSION["target_show_detail"] = $_REQUEST["target_show_detail"];
        if ( isset ( $_REQUEST["target_show_graph"] ))  $_SESSION["target_show_graph"] = $_REQUEST["target_show_graph"];
        if ( isset ( $_REQUEST["target_show_group_headers"] ))  $_SESSION["target_show_group_headers"] = $_REQUEST["target_show_group_headers"];
        if ( isset ( $_REQUEST["target_show_group_trailers"] ))  $_SESSION["target_show_group_trailers"] = $_REQUEST["target_show_group_trailers"];
        if ( isset ( $_REQUEST["target_show_column_headers"] ))  $_SESSION["target_show_column_headers"] = $_REQUEST["target_show_column_headers"];
        if ( isset ( $_REQUEST["target_show_criteria"] ))  $_SESSION["target_show_criteria"] = $_REQUEST["target_show_criteria"];
	...
	function login_check($smarty)
	{
		...
		if (
			...
			( isset($_SESSION['project_password']) && $_SESSION['project_password'] == SW_PROJECT_PASSWORD && $this->allow_maintain == "DEMO" )

		)
		{
            // After logging on to project allow user access to design mode if user is admin or if we
            // are running in "DEMO" mode
			if ( isset($_SESSION['admin_password'] ) || $this->allow_maintain == "DEMO" )
				$loggedon = "DESIGN";
			else
				$loggedon = "NORMAL";
		}
	...
	function initialize_panels($mode)
	{
        ...
        // Derive full "http://x.x.x.x" host which would need to prepend a reportico
        // call - used for creating PDF link elements
        if ( !defined('SW_HTTP_URLHOST') )
			define ( 'SW_HTTP_URLHOST', "http://127.0.0.1" );

        // Derive URL path to the reportico instance
        if ( !defined('SW_USESELF') )
			define ( 'SW_USESELF', $_SERVER["PHP_SELF"]);

		$useself = session_request_item('linkbaseurl', SW_USESELF);
        $reportico_url_path = get_reportico_url_path();

        // Set full URL path required to get to reportico
        if ( !defined("SW_HTTP_PATH_TO_REPORTICO" ) )
            define("SW_HTTP_PATH_TO_REPORTICO", SW_HTTP_URLHOST. $reportico_url_path);
		...
		$smarty->assign('AJAX_PARTIAL_RUNNER', $reportico_url_path.$partialajaxpath );

        $csspath = find_best_location_in_include_path( SW_STYLESHEET );
		$smarty->assign('STYLESHEET', $reportico_url_path.$csspath);

        $jspath = find_best_location_in_include_path( "js/reportico.js" );
		if ( $jspath ) $jspath = dirname($jspath);

		$smarty->assign('JSPATH', $reportico_url_path.$jspath);
		...
		$this->prepare_url =  session_request_item('linkbaseurl', SW_USESELF)."?execute_mode=PREPARE&amp;session_name=".session_id();
		...
		$this->delete_project_url =  session_request_item('linkbaseurl', SW_USESELF)."?execute_mode=PREPARE&amp;xmlin=deleteproject.xml&amp;session_name=".session_id();
...

		{
			...
			if ( $login_type = $this->login_check($smarty) )
			{
				...
				if ( $login_type == "DESIGN" || $login_type == "ADMIN" ||
					( defined ('SW_PROJECT_PASSWORD') && SW_PROJECT_PASSWORD != '' ) )
...
//echo "User temp ".$this->user_template."!<BR>";
	...
	function execute($mode=false, $draw=true)
	{
		...
		switch ( $mode )
		{
				case "PREPARE":
                    ...
                    if ( !isset($_SESSION["firstTimeIn"])  )
                        $_SESSION["firstTimeIn"] = true;

                    // Default output to HTML in PREPARE mode first time in
                    if ( $_SESSION["firstTimeIn"] && !isset($_REQUEST["target_format"]))
                    {
		...
		{
			if ( $mode != "MODIFY" && array_key_exists('latestRequest', $_SESSION ))
			{
		...
		switch ($mode)
		{
			...
			case "PREPARE":
                ...
                restore_error_handler();
				if ($this->user_template == "fullscreen" && preg_match("/$reportname/", find_best_location_in_include_path( "templates/". $reportname )))
				{
//echo "Disp ".$reportname."!<BR>";
					$this->panels["MAIN"]->smarty->display($reportname);
				}
				else
				if ( $this->user_template )
                {
//echo "Disp1 ".$this->user_template."!<BR>";
...
//echo "Disp2 "."prep!!!"."!<BR>";
	...
	function execute_query($in_criteria_name)
	{
		...
		{
            $http_error_no_data_reponse = derive_config_report_params(preg_replace("/\.xml$/", "", $this->xmlinput), "http_no_data_message", false);
            if ( $http_error_no_data_reponse )
            {
                header("HTTP/1.0 404 ".$http_error_no_data_reponse, true);
                die;
            }
		}
	...
	// -----------------------------------------------------------------------------
	function embed_image($column_assignee, $image)
	{
    ...
    // Generates a link object against a column
	// -----------------------------------------------------------------------------
	function embed_hyperlink($column_assignee, $label, $url, $open_in_new = true, $styles = false)
	{
            get_query_column($column_assignee, $this->columns )->output_hyperlinks =
                        array("label" => $label, "url" => $url, "open_in_new" => $open_in_new, "styles" => $styles);
	}
...
class reportico_criteria_column extends reportico_query_column
{
		...
		if ( AJAX_ENABLED )
		{
			$tag = "";
       			...
       			if ( preg_match ( "/TODATE/", $in_tag ) )
              			$tag .= "";
            $tag .= '<input  class="swDateField" id="swDateField_'.$in_tag."_".session_id().'" type="text" name="MANUAL_'.$in_tag.'"';
	...
	function & list_display($in_is_expanding)
	{
		...
		if ( !array_key_exists("clearform", $_REQUEST) )
		{
			...
			// If this is first time into screen and we have defaults then
			// use these instead
			if ( !$params && !$hidden_params && $_SESSION["firstTimeIn"] )
			{
	...
	function & lookup_display($in_is_expanding)
	{
		...
		if ( !array_key_exists("clearform", $_REQUEST) )
		{
			...
			// If this is first time into screen and we have defaults then
			// use these instead
			if ( !$hidden_params && $_SESSION["firstTimeIn"] )
			{
				$hidden_params = $this->defaults;
				$manual_params = $this->defaults;
			}
...
class reportico_assignment extends reportico_object
{
	...
	{
        global $framework_user;
        if ( $framework_user )
        {
            $in_string = preg_replace ("/USER/", "'".$framework_user."'", $in_string);
	    }
        ...
        // statement. Just include {SESSION_USER} in queries and it will be
...
function set_project_environment()
{
    ...
    // replaced by a quote encased user.
    global $framework_user;
    $framework_user = session_request_item("user", false);
	...
	if ( defined('SW_LANGUAGE') && SW_LANGUAGE && SW_LANGUAGE != "PROMPT" )
	    $language = session_request_item("language", SW_LANGUAGE);
    else
	    $language = session_request_item("language", "en_gb");
