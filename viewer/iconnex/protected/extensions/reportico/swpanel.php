 ...
 * @package Reportico
 * @version $Id: swpanel.php,v 1.27 2012-09-02 19:20:03 root Exp $
 */
	...
	function pre_draw_smarty()
	{
		...
		switch($this->panel_type)
		{
			...
			case "DESTINATION":
				...
				$this->smarty->assign("OUTPUT_SHOWGRAPH", $_SESSION["target_show_graph"] ? "checked" : "" );
				$this->smarty->assign("OUTPUT_SHOWCRITERIA", $_SESSION["target_show_criteria"] ? "checked" : "" );
				$this->smarty->assign("OUTPUT_SHOWDETAIL", $_SESSION["target_show_detail"] ? "checked" : "" );
				$this->smarty->assign("OUTPUT_SHOWGROUPHEADERS", $_SESSION["target_show_group_headers"] ? "checked" : "" );
				$this->smarty->assign("OUTPUT_SHOWGROUPTRAILERS", $_SESSION["target_show_group_trailers"] ? "checked" : "" );
				$this->smarty->assign("OUTPUT_SHOWCOLHEADERS", $_SESSION["target_show_column_headers"] ? "checked" : "" );
...
class reportico_xml_reader
{
  	...
  	function reportico_xml_reader (&$query, $filename, $xmlstring = false, $search_tag = false ) 
	{
    	...
    	$this->field_display = array (
					...
					"SQLLimitFirst" => array ( "Title" => "SQLLIMITFIRST" , "Validate" => "NUMBER"),
					"SQLSkipOffset" => array ( "Title" => "SQLSKIPOFFSET", "Validate" => "NUMBER" ),
	...
	function update_maintain_fields ($match) 
	{
			...
			switch ( $anal["action"] )
			{
				case "sqlt":
					...
					$updateitem =& $anal["item"];
                    if ( isset($updates["SQLLimitFirst"]) )
					    $updateitem->sql_limit_first = $updates["SQLLimitFirst"];
                    if ( isset($updates["SQLSkipOffset"]) )
					    $updateitem->sql_skip_offset = $updates["SQLSkipOffset"];
				...
				case "assg":
					...
					if ( $updates["AssignHyperlinkLabel"] ) 
                    {
                        ...
                        $hurl = preg_replace("/}/", "}.'", $hurl);
                        $updates["Expression"] =  "embed_hyperlink(".$hlabel.", ".$hurl.");";
                    }
					...
					if ( $updates["DrilldownReport"] )
					{
                            ...
                            $content = "Drill";
                            $url = session_request_item('linkbaseurl', SW_USESELF)."?xmlin=".$updates["DrilldownReport"]."&execute_mode=EXECUTE&target_format=HTML&target_show_body=1&project=".$g_project;
							...
							unset($q);
							if ( $midbit )
								$updates["Expression"] = "embed_hyperlink('".$content."', '". $url. "')";
	...
	function & draw_add_button ($in_tag, $in_value = false) 
	{
		$text = "";
		$text .= '<TD>';
		$text .= '<input class="swMntButton" type="submit" name="submit_'.$in_tag.'_ADD" value="'.template_xlate("ADD").'">';
		$text .= '</TD>';
	...
	function & draw_report_link_panel ($link_or_import, $tag, $label, $preselectedvalue = false) 
	{
        ...
        $text .= '<input class="swMntButton" style="margin-right: 20px" type="submit" name="submit_'.$this->id.'_REPORTLINK" value="'.template_xlate("OK").'">';
        ...
        if ( $this->query->reportlink_report )
        {
            ...
            else
            {
                ...
                $text .= '<input class="swMntButton" style="margin-right: 20px" type="submit" name="submit_'.$this->id.'_REPORTLINKITEM" value="'.template_xlate("OK").'">';
            }
        }
			...
			if ( is_array($val) )
			{
				...
				switch ( $k )
				{
					...
					case "CogModule":
						...
						$text .= '&nbsp;&nbsp;<input class="swLinkMenu" type="submit" name="submit_xxx_SAVE" value="'.template_xlate("SAVE").'">';
						$text .= '&nbsp;&nbsp;<input class="swLinkMenu" type="submit" name="submit_maintain_NEW" value="'.template_xlate("NEW_REPORT").'">';
						$text .= '<input class="swLinkMenu" style="background-color: #ffcccc; margin-left: 80px" type="submit" name="submit_xxx_DELETEREPORT" value="'.template_xlate("DELETE_REPORT").'">';
						$text .= '</TD>';
						$text .= '</TR>';
						//$text .= '<TR>';
						break;
	...
	function & display_maintain_field($tag, $val, &$tagct, $translate = true, $overridetitle = false, $toggleclass = false, $togglestate = false)
	{
		...
		if ( $tagct == 1 )
		{
			...
			if ( $type != "TEXTFIELDNOOK" )	
				$text .= '<input class="swMntButton" type="submit" name="submit_'.$this->id.'_SET" value="'.template_xlate("OK").'">';
			else
				$text .= "&nbsp;";
			$text .= '</TD>';
		}
    ...
    */
	function xml2query () 
	{
		...
		foreach ( $q as $cogquery )
		{
			...
			{
			    $this->query->sql_limit_first = $this->get_array_element($qc,"SQLLimitFirst");
			    $this->query->sql_skip_offset = $this->get_array_element($qc,"SQLSkipOffset");
...
class reportico_xml_writer
{
	...
	function prepare_xml_data()
	{
		...
		$el =& $sq->add_xmlval ( "SQLLimitFirst", $this->query->sql_limit_first);
		$el =& $sq->add_xmlval ( "SQLSkipOffset", $this->query->sql_skip_offset);
